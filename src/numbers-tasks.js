...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthProduct = Math.sqrt((x1 ** 2 + y1 ** 2) * (x2 ** 2 + y2 ** 2));
  return Math.acos(scalar / lengthProduct);
}
...
function getSumOfDigits(num) {
  return `${num}`.split('').reduce((total, el) => +el + total, 0);
}
...
function getSumOfNumbers(x1, x2, x3) {
  const precision1 = ''.concat(x1 - Math.floor(x1)).length;
  const precision2 = ''.concat(x2 - Math.floor(x2)).length;
  const precision3 = ''.concat(x3 - Math.floor(x3)).length;
  const maxPrecision = Math.max(precision1, precision2, precision3);
  return (x1 + x2 + x3).toPrecision(maxPrecision);
}
...
function getCountOfOddNumbers(number) {
  return Math.floor(Math.abs(number) / 2) + (number % 2 ? 1 : 0);
}
